...
const METHODS = [
  { icon: '✉️', label: 'Email', value: 'quantpos@gmail.com', href: 'mailto:quantpos@gmail.com', desc: 'Best for general questions', color: '#00A4A4' },
      ...
      <PublicLayout fullWidth>
        ...
        <div className="min-h-[70vh] flex items-center justify-center px-6">
          <div className="text-center max-w-md" style={{ animation: 'qp-scale-in .5s cubic-bezier(.22,1,.36,1) both' }}>
            <div className="w-20 h-20 rounded-full flex items-center justify-center text-4xl mx-auto mb-6 shadow-2xl shadow-[#00A4A4]/30"
              style={{ background: 'linear-gradient(135deg, #00A4A4, #10b981)' }}>
              ✓
            </div>
      ...
      <section className="relative overflow-hidden pt-28 pb-12 px-6 qp-grid-bg">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          <div className="qp-reveal">
            <GradientBadge color="#00A4A4">Get in touch</GradientBadge>
            ...
            <div className="space-y-4">
              {METHODS.map(m => (
                <a key={m.label} href={m.href} target="_blank" rel="noreferrer"
                  className="flex items-center gap-4 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 hover:border-accent/40 transition-all duration-200 group">
              ...
              <div className="flex flex-wrap gap-2">
                {TOPICS.map(t => (
                  <button key={t.label} onClick={() => setTopic(t.label)}
                    ...
                    style={{
                      background: topic === t.label ? '#00A4A4' : 'transparent',
                      color: topic === t.label ? '#fff' : 'var(--text-muted)',
                      borderColor: topic === t.label ? '#00A4A4' : 'var(--border)',
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {[
                  ...
                  <div key={f.name}>
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-[var(--border)] bg-[var(--bg)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none transition-all duration-150 focus:border-accent focus:ring-2 focus:ring-accent/20"
                    />
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-[var(--border)] bg-[var(--bg)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder-[var(--text-muted)] outline-none transition-all duration-150 focus:border-accent focus:ring-2 focus:ring-accent/20 resize-none"
                />
              </div>

              <button type="submit"
                className="qp-btn-primary w-full bg-accent text-white font-bold py-4 rounded-xl text-base shadow-xl shadow-[#00A4A4]/30 flex items-center justify-center gap-2">
                Send message
