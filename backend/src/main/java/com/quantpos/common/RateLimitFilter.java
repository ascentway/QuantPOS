 ...
 *
 * <p>Uses Redis INCR + EXPIRE (already implemented in RedisService.increment).
 * The TTL is set on first write (window start); subsequent increments within the
 * same window do NOT reset the timer — giving a true fixed-window counter.
 *
 * <p>Redis key format:  rate_limit:{ip}:{endpoint_slug}  → count  (TTL = window seconds)
 *
 * <p>Rate matrix (applies to ALL profiles including dev):
 * <ul>
