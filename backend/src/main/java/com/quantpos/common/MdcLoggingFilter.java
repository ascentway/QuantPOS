 ...
 * <ul>
 *   <li>{@code requestId}  — UUID, either taken from incoming X-Request-ID header or freshly generated</li>
 *   <li>{@code method}     — HTTP method (GET, POST, …)</li>
 *   <li>{@code path}       — Request URI</li>
 * </ul>
 *
 * <p>Additional MDC keys ({@code userId}, {@code tenantId}) are set later by
 * {@link com.quantpos.auth.security.JwtFilter} once the JWT is validated.
 *
 * <p>The requestId is echoed back in the {@code X-Request-ID} response header so
 * the frontend/API client can correlate frontend logs with backend traces.
 *
 * <p>MDC is always cleared in the {@code finally} block to prevent ThreadLocal leaks.
 */
@Component
