...
public class StripeWebhookService {
    ...
    public boolean processEvent(Event event, String rawPayload) {
        ...
        try {
            ...
            log.error("Failed to process Stripe event | eventId={} type={} error={}", eventId, eventType, e.getMessage(), e);
    ...
    private void handleCheckoutCompleted(Event event, PaymentEvent paymentEvent) {
        extractObject(event, com.stripe.model.checkout.Session.class).ifPresent(session -> {
                ...
                BigDecimal amount = session.getAmountTotal() != null ? BigDecimal.valueOf(session.getAmountTotal()) : BigDecimal.ZERO;
    ...
    private void handleInvoiceSucceeded(Event event, PaymentEvent paymentEvent) {
        extractObject(event, com.stripe.model.Invoice.class).ifPresent(invoiceObj -> {
                ...
                BigDecimal amount = invoiceObj.getAmountPaid() != null ? BigDecimal.valueOf(invoiceObj.getAmountPaid()) : BigDecimal.ZERO;
    ...
    private void handleSubscriptionUpdated(Event event, PaymentEvent paymentEvent) {
        extractObject(event, com.stripe.model.Subscription.class).ifPresent(sub -> {
            ...
            findTenantByCustomerId(customerId).ifPresent(tenant -> {
                ...
                Subscription subscription = subscriptionRepository.findByStripeSubscriptionId(sub.getId()).orElse(null);
                if (subscription != null) {
                    subscription.setCancelAtPeriodEnd(sub.getCancelAtPeriodEnd() != null ? sub.getCancelAtPeriodEnd() : false);
                    ...
                    if (sub.getCurrentPeriodStart() != null) {
                        subscription.setCurrentPeriodStart(LocalDate.ofInstant(java.time.Instant.ofEpochSecond(sub.getCurrentPeriodStart()), java.time.ZoneId.systemDefault()));
                    }
                    if (sub.getCurrentPeriodEnd() != null) {
                        subscription.setCurrentPeriodEnd(LocalDate.ofInstant(java.time.Instant.ofEpochSecond(sub.getCurrentPeriodEnd()), java.time.ZoneId.systemDefault()));
                        subscription.setNextBillingDate(subscription.getCurrentPeriodEnd());
                    }
                    if (sub.getTrialStart() != null) {
                        subscription.setTrialStart(LocalDate.ofInstant(java.time.Instant.ofEpochSecond(sub.getTrialStart()), java.time.ZoneId.systemDefault()));
                    }
                    if (sub.getTrialEnd() != null) {
                        subscription.setTrialEnd(LocalDate.ofInstant(java.time.Instant.ofEpochSecond(sub.getTrialEnd()), java.time.ZoneId.systemDefault()));
                    }
                    ...
                    if (sub.getItems() != null && sub.getItems().getData() != null && !sub.getItems().getData().isEmpty()) {
                        ...
                        if (item.getPrice() != null && item.getPrice().getId() != null) {
                            ...
                            if (item.getPrice().getUnitAmount() != null) {
                                subscription.setMonthlyPrice(BigDecimal.valueOf(item.getPrice().getUnitAmount()).divide(BigDecimal.valueOf(100)));
                            }
                        }
                    }

                    subscriptionRepository.save(subscription);
                    log.info("Tenant subscription updated | tenantId={} subscriptionId={} cancelAtPeriodEnd={}", tenant.getId(), sub.getId(), sub.getCancelAtPeriodEnd());
                }
            });
        });
    }
    ...
    private void handleInvoiceFailed(Event event, PaymentEvent paymentEvent) {
        extractObject(event, com.stripe.model.Invoice.class).ifPresent(invoiceObj -> {
                ...
                findTenantByCustomerId(customerId).ifPresent(tenant -> {
                    ...
                    log.warn("Tenant invoice payment failed | tenantId={} invoiceId={}", tenant.getId(), invoiceObj.getId());
                });
        });
    }
    ...
    private void saveOrUpdateSubscription(Tenant tenant, com.stripe.model.Subscription stripeSub, String providedPlanType) {
        ...
        subscription.setCurrentPeriodStart(LocalDate.ofInstant(Instant.ofEpochSecond(stripeSub.getCurrentPeriodStart()), ZoneId.systemDefault()));
        subscription.setCurrentPeriodEnd(LocalDate.ofInstant(Instant.ofEpochSecond(stripeSub.getCurrentPeriodEnd()), ZoneId.systemDefault()));
        
        String stripeStatus = stripeSub.getStatus().toUpperCase();
        SubscriptionStatus mappedStatus = SubscriptionStatus.INACTIVE;
        if (stripeStatus.equals("ACTIVE")) mappedStatus = SubscriptionStatus.ACTIVE;
        else if (stripeStatus.equals("PAST_DUE")) mappedStatus = SubscriptionStatus.PAST_DUE;
        else if (stripeStatus.equals("CANCELED") || stripeStatus.equals("CANCELLED")) mappedStatus = SubscriptionStatus.CANCELLED;
        ...
        if (stripeSub.getTrialStart() != null) {
            subscription.setTrialStart(LocalDate.ofInstant(Instant.ofEpochSecond(stripeSub.getTrialStart()), ZoneId.systemDefault()));
        }
        if (stripeSub.getTrialEnd() != null) {
            subscription.setTrialEnd(LocalDate.ofInstant(Instant.ofEpochSecond(stripeSub.getTrialEnd()), ZoneId.systemDefault()));
        }
    ...
    private void saveInvoice(Tenant tenant, com.stripe.model.Invoice stripeInvoice, String status) {
        BigDecimal amountDue = stripeInvoice.getAmountDue() != null ? BigDecimal.valueOf(stripeInvoice.getAmountDue()).divide(BigDecimal.valueOf(100)) : BigDecimal.ZERO;
        BigDecimal amountPaid = stripeInvoice.getAmountPaid() != null ? BigDecimal.valueOf(stripeInvoice.getAmountPaid()).divide(BigDecimal.valueOf(100)) : BigDecimal.ZERO;
        ...
        Invoice invoice = Invoice.builder()
                ...
                .invoiceDate(stripeInvoice.getCreated() != null ? LocalDate.ofInstant(Instant.ofEpochSecond(stripeInvoice.getCreated()), ZoneId.systemDefault()) : null)
                .dueDate(stripeInvoice.getDueDate() != null ? LocalDate.ofInstant(Instant.ofEpochSecond(stripeInvoice.getDueDate()), ZoneId.systemDefault()) : null)
                .status(status)
    ...
    private void handleTrialWillEnd(Event event, PaymentEvent paymentEvent) {
        extractObject(event, com.stripe.model.Subscription.class).ifPresent(sub -> {
            ...
            findTenantByCustomerId(customerId).ifPresent(tenant -> {
                paymentEvent.setTenant(tenant);
                log.info("Tenant subscription trial will end soon | tenantId={} subscriptionId={}", tenant.getId(), sub.getId());
            });
        });
    }
    ...
    private String resolvePlanTypeFromPriceId(String priceId) {
        ...
        if (priceId.equals(stripeConfig.getStarterPriceMonthly()) || priceId.equals(stripeConfig.getStarterPriceYearly())) {
            return "STARTER";
        }
        if (priceId.equals(stripeConfig.getGrowthPriceMonthly()) || priceId.equals(stripeConfig.getGrowthPriceYearly())) {
            return "GROWTH";
        }
        if (priceId.equals(stripeConfig.getProfessionalPriceMonthly()) || priceId.equals(stripeConfig.getProfessionalPriceYearly())) {
            return "PROFESSIONAL";
        }
        if (priceId.equals(stripeConfig.getEnterprisePriceMonthly()) || priceId.equals(stripeConfig.getEnterprisePriceYearly())) {
            return "ENTERPRISE";
        }
        return null;
    }
